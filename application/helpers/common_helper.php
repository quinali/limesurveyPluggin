...
/**
* JNL
* Recupera todas las questions y subquestions para un determinado SID
* y los devuelve en un array de options para introducir
* directamente en un select
*/
function getQuestionsForTokens($surveyid,$selectedqid)
{
	$s_lang = Survey::model()->findByPk($surveyid)->language;
    //$qrows = Question::model()->findAllByAttributes(array('sid' => $surveyid, 'language' => $s_lang),array('order'=>'question_order'));
	
	$qrows = Question::model()->findAllByAttributes(array('sid' => $surveyid, 'language' => $s_lang),array('order'=>'IF(parent_qid=0,qid,concat(parent_qid,qid))'));
    ...
    foreach ($qrows as $qrow)
    {
		...
		if($qrow['parent_qid'] ==0){
			$value =$surveyid."X".$qrow['gid']."X".$qrow['qid'];
		}else{
			$value =$surveyid."X".$qrow['gid']."X".$qrow['parent_qid'].$qrow['title']; ;
		}
		
		
		$sQuestionselecter .= "<option value='{$value}'";
    ...
    if (!isset($qexists))
    {
        $sQuestionselecter = "<option selected='selected'>".gT("(none)")."</option>\n".$sQuestionselecter;
    }
    else
    {
        $link = Yii::app()->getController()->createUrl("/admin/survey/sa/view/surveyid/".$surveyid."/gid/".$gid);
